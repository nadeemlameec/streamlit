...
def compute_kpis(df, cols):
    ...
    if rev and spend and insights.get("total_spend", 0) != 0:
...
if uploaded_file:
    ...
    if uploaded_file.type == "text/csv":
        uploaded_file.seek(0)                          # FIX: was uploaded_[file.seek](...)
        raw_df = pd.read_csv(uploaded_file)            # FIX: was [pd.read](...)_csv
        ...
        if uploaded_file.type == "application/pdf" and PdfReader:
            ...
            file_content = uploaded_file.read().decode("utf-8")  # FIX: was uploaded_[file.read](...)
...
if st.session_state.df is not None:
    ...
    if perf is not None:
        ...
        st.bar_chart(perf.set_index(perf.columns[0]))  # FIX: was [st.bar](...)_chart
...
for m in st.session_state.messages:
    with st.chat_message(m["role"]):              # FIX: was [st.chat](...)_message
        st.markdown(m["content"])

if q := st.chat_input("Ask..."):                  # FIX: was [st.chat](...)_input
    st.session_state.messages.append({"role": "user", "content": q})
    with st.chat_message("assistant"):            # FIX: was [st.chat](...)_message
        context = safe_context(
            st.session_state.df if st.session_state.df is not None else file_content
        )
        try:
